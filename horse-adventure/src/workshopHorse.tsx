...
export type WorkshopHorseAction = "walk" | "stand" | "eat" | "jiggle_ears";

type WorkshopHorseDisplay = {
  showSkeleton?: boolean;
  wireframe?: boolean;
};

export const WORKSHOP_RABBIT_POSITION: [number, number, number] = [3.5, 0, 0];

const WORKSHOP_RABBIT_BODY_HEIGHT = 0.6;
const WORKSHOP_RABBIT_EAR_TOP = 1.1;
const WORKSHOP_HORSE_COLOR = "#c58b62";
const WORKSHOP_HORSE_LIFT = 0.18;
const WORKSHOP_TORSO_HEIGHT = WORKSHOP_RABBIT_BODY_HEIGHT;
const WORKSHOP_TORSO_LENGTH = WORKSHOP_TORSO_HEIGHT * 2.5;
const WORKSHOP_TORSO_DEPTH = WORKSHOP_TORSO_HEIGHT;
const WORKSHOP_TORSO_CENTER_Y = WORKSHOP_RABBIT_EAR_TOP + WORKSHOP_TORSO_HEIGHT * 0.5 + WORKSHOP_HORSE_LIFT;

const WORKSHOP_HORSE_PARTS: BlockPart[] = [
  { id: "torso", position: [0, WORKSHOP_TORSO_CENTER_Y, 0], size: [WORKSHOP_TORSO_LENGTH, WORKSHOP_TORSO_HEIGHT, WORKSHOP_TORSO_DEPTH] },
...
function WorkshopHorseBlock({ part, wireframe = true }: { part: BlockPart; wireframe?: boolean }) {
  return (
    <mesh castShadow receiveShadow>
      <boxGeometry args={part.size} />
      <meshStandardMaterial color={part.color ?? WORKSHOP_HORSE_COLOR} roughness={1} flatShading wireframe={wireframe} />
    </mesh>
  );
}

function WorkshopSkeletonConnector({ to }: { to: [number, number, number] }) {
...
export function WorkshopHorse({ showSkeleton = true, wireframe = true, action = "stand" }: WorkshopHorseDisplay & { action?: WorkshopHorseAction }) {
  const partRefs = useRef(new Map<string, Group>());
  const childrenByParent = useMemo(() => {
    const entries = new Map<string | undefined, BlockPart[]>();

    for (const part of WORKSHOP_HORSE_PARTS) {
      const siblings = entries.get(part.parent);
      if (siblings) {
        siblings.push(part);
      } else {
        entries.set(part.parent, [part]);
      }
    }
  ...
  const renderNode = (part: BlockPart): ReactNode => {
    ...
    return (
      <group
        ...
        ref={(node) => {
          if (node) {
            partRefs.current.set(part.id, node);
          } else {
            partRefs.current.delete(part.id);
          }
        }}
      >
        <WorkshopHorseBlock part={part} wireframe={wireframe} />
        {showSkeleton ? <mesh><boxGeometry args={[0.1, 0.1, 0.1]} /><meshBasicMaterial color="#ff8fd1" /></mesh> : null}
        {showSkeleton ? children.map((child) => <WorkshopSkeletonConnector key={`connector-${child.id}`} to={child.position} />) : null}
